// Phase J.2 — city article index.
//
// URL: /{locale}/{city}  e.g. /el/athens
//
// The segment falls through here only when it's not one of the
// reserved static routes (/auth, /cities, /claim, /dashboard, /greece,
// /guides, /legal, /review, /search, /sign-in, /sites, /for-owners,
// /featured, /go). A server-side getCityBySlug() guard turns anything
// else into a 404.
...
function ArticleCard({ article, locale, citySlug }: { article: Article; locale: string; citySlug: string }) {
  return (
    <li>
      <Link
        href={`/${locale}/${citySlug}/${article.slug}`}
